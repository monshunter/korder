...
var _ = BeforeSuite(func() {
	...
	// Setup controllers
	err = (&controller.OrderReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())

	err = (&controller.TicketReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())

	err = (&controller.QuotaReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())
	...
	// Setup webhooks
	err = webhookv1.SetupPodWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())

	err = webhookv1alpha1.SetupOrderWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())

	err = webhookv1alpha1.SetupTicketWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())

	err = webhookv1alpha1.SetupQuotaWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())
